...
import com.carparking.application.service.TicketTypeService;
...
public class TicketTypeController {
  ...
  public ResponseEntity<?> getAll() throws Exception {
    ...
    List<TicketTypeModel> data = ticketTypeService.getAll();
  ...
  public ResponseEntity<?> getPage(@RequestParam(name = "size", required = false, defaultValue = "10") int size,
    ...
    Map<String, Object> response = new HashMap<>();
    ResponseObjectType<TicketTypeModel> data = ticketTypeService.getPageSize(keyword, page, size, field, sort);
  ...
  @PostMapping
  @CrossOrigin(origins = "*", maxAge = 3600)
  public ResponseEntity<?> create(@RequestBody @Valid TicketTypeRequest request) throws Exception {
    Map<String, Object> response = new HashMap<>();
    TicketTypeModel data = ticketTypeService.create(request);
    response.put("data", data);
  ...
  public ResponseEntity<?> getById(@PathVariable(value = "id") Long id) throws Exception {
    Map<String, Object> response = new HashMap<>();
    response.put("data", ticketTypeService.findById(id));
    response.put("status", 200);
    response.put("message", "Get data successfully!");
    return new ResponseEntity<>(response, HttpStatus.OK);
  }
  ...
  @PutMapping("{id}")
  @CrossOrigin(origins = "*", maxAge = 3600)
  public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @Valid @RequestBody TicketTypeRequest request)
      throws Exception {
    Map<String, Object> response = new HashMap<>();
    TicketTypeModel data = ticketTypeService.update(id, request);
    response.put("data", data);
    response.put("status", 200);
